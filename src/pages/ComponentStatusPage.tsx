					...
					<React.Fragment key={category.category}>
						...
						<div className="overflow-x-auto mt-3">
							<table className="w-full text-sm text-left rtl:text-right text-gray-500">
								...
								<tbody>
									{[...category.components]
										?.sort((a: IComponentApi, b: IComponentApi) => a.name.localeCompare(b.name))
										.map((component, idx) => (
											<tr key={idx + component.name} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-slate-100'}`}>
												<th scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap text-center">
													{component.name}
												</th>
												<td className="px-6 py-4 text-center">{component.statuses[0].guidelines}</td>
												<td className="px-6 py-4 text-center">{component.statuses[0].figma}</td>
												<td className="px-6 py-4 text-center">{component.statuses[0].storybook}</td>
