...
const DEFAULT_VALUES: IComponentApi = {
	...
	updatedAt: '',
	status: [
		{ platformName: 'guidelines', stage: '' },
		{ platformName: 'figma', stage: '' },
		{ platformName: 'storybook', stage: '' },
		{ platformName: 'cdn', stage: '' }
	]
};
...
export const ComponentLayout: React.FC<ComponentLayoutProps> = ({ children, className = '', statusBar = true }) => {
	...
	const handleEdit = useCallback(() => {
		if (id) {
			setSelectedRecord(state as unknown as IComponentApi);
			toggleModal();
		}
	...
	return (
		<main className={`container mx-auto ${className}`} data-testid="component-layout">
			<header>
				{categoryName && <small className="text-sm text-primary-blue block">{categoryName}</small>}
