...
export default function SearchBar() {
	...
	useEffect(() => {
		const fetchComponents = async () => {
			try {
				...
				const componentNames = data.map((component: { name: string }) => component.name);
	...
	const filteredComponents = components.filter((item) => item.toLowerCase().includes(query.toLowerCase()));
	...
	const navigateToComponent = (componentName: string) => {
		...
		if (routesIndex[1].children?.some((route) => route.path === formattedName)) {
			handleDataSend(navigate, `/docs/${formattedName}`, componentName, categories ?? []);
		} else {
			handleDataSend(navigate, `/docs/WipComponent/${formattedName}`, componentName, categories ?? []);
		}
	};
